...
public:
  ...
  inline Eigen::Vector2i world2map(const Eigen::Vector2d &pos, const Global_Map &gm);
  inline bool isValid(const Eigen::Vector2i &idx, const Global_Map &gm);
...
private:
  ...
  std::shared_ptr<tf2_ros::MessageFilter<sensor_msgs::msg::PointCloud2>> cloud_filter_;
  ...
  void cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg_in);
...
inline Eigen::Vector2i SDFMap::world2map(const Eigen::Vector2d &pos, const Global_Map &gm)
{
    Eigen::Vector2i idx;
    idx(0) = floor((pos(0) - gm.map_origin_(0)) / gm.resolution_);
    idx(1) = floor((pos(1) - gm.map_origin_(1)) / gm.resolution_);
    return idx;
}

inline bool SDFMap::isValid(const Eigen::Vector2i &idx, const Global_Map &gm)
{
